...
void addMidiNoteToBuffer(int note, double time, double length, int *buf, int vel)
{
    ...
    for(int i=0; i<length*sampleRate; i++)
    {
        ...
        float sample=16000*vol*sin((2.f*float(M_PI)*freq)/sampleRate * i);
        if (sample>0)
        {
...
void addSynthNoteToBuffer(double freq, double time, double length, int*buf, double vol)
{
    int startSamp=(int)(time*sampleRate);
    double noteSound;
    double envolope;
...
void perlinSynthSong(float length, float minFreq, float maxFreq, int intervals, int notes, int* buf);
...
int main(int argc, char** argv) {
    ...
    int note;
    int vel;
    double songLength=midifile.getTimeInSeconds(track, midifile.getEventCount(0)-1);
    songLength=900.0*60/100+5;
    //printf("%f\n", songLength);
    ...
    for (int i=0; i<midifile[track].size(); i++)
    {
        ...
        {
            time=midifile.getTimeInSeconds(track, i);
            length=midifile[track][i].getDurationInSeconds();
            note=(int)midifile[track][i][1];
            vel=(int)midifile[track][i][2];
            printf("Note: %d\nVel: %d\n", note, vel);
            addMidiNoteToBuffer(note, time, length, samples, vel);
        }
    }
    
    addReverb(buf_size, .3, .6, .4, samples);
    
    //perlinSynthSong(0, 0, 0, 0, 0, samples);
    
    convertIntBufToShort(buf_size, samples, samplesShort);
    float relSpeed=1.0;
    /* Download buffer to OpenAL */
    alBufferData(buf, AL_FORMAT_MONO16, samplesShort, buf_size, relSpeed*sampleRate);
    al_check_error();
...
void perlinSynthSong(float length, float minFreq, float maxFreq, int intervals, int notes, int* buf)
{
    ...
    unsigned short* data=(unsigned short*)malloc(width*height*pixelBitSize);
    float* heightData=(float*)malloc(width*height*sizeof(float));
    float* octaveNoise=(float*)malloc(width*height*sizeof(float));
    genNoiseFloat(width, height, initHorDiv, initVertDiv, octaves, .5, heightData, octaveNoise, 2);
...
float freqForMidiNote(int note)
{
    return 8.175799*powf(2, note/12.0);
}
